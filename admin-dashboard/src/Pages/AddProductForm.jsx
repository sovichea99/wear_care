...
export default function AddProductForm({ onAdd, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Always use FormData since we might have an image
      const formData = new FormData();
      
      // Append all fields with the correct names
      formData.append('name', newProduct.name);
      formData.append('price', Number(newProduct.price));
      formData.append('category_id', newProduct.category_id);
      formData.append('description', newProduct.description || '');
      
      // Append variants in the format that handleAddProduct expects
          ...
          <form id="add-product-form" onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              {duplicateSizeWarning && (
                <div className="text-yellow-600 text-xs mb-1">{duplicateSizeWarning}</div>
              )}
